...
class FrameworkDetector:
    def detect(self, root, files, dependencies):
        ...
        deps = {d.name.lower() for d in dependencies}
        found = []
        rules = [("Django", {"django"}, {"manage.py"}), ("Flask", {"flask"}, set()), ("FastAPI", {"fastapi"}, set()), ("React", {"react"}, set()), ("Next.js", {"next"}, set()), ("Vue", {"vue"}, set()), ("Express", {"express"}, set())]
        for name, depnames, markers in rules:
...
class ConfigurationDetector:
    def detect(self, files):
        return sorted({f.path for f in files if Path(f.path).name.lower() in CONFIG_NAMES or Path(f.path).name.lower().startswith(".env")})

class EntryPointDetector:
    def detect(self, root, files):
        names={f.path for f in files}; found=[]
        for p in ("manage.py","main.py","app.py","server.py","index.js","index.ts","src/main.py","src/main.ts","src/index.ts","src/index.js","wsgi.py","asgi.py"):
            if p in names: found.append(p)
        return found

class TestFrameworkDetector:
    def detect(self, files, dependencies):
        names={d.name.lower() for d in dependencies}; paths=[f.path.lower() for f in files]
        found=[]
